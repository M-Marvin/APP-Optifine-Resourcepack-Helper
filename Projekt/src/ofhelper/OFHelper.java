...
public class OFHelper {
	...
	public static void main(String... args) {
		...
		OFHelper ofHelper = new OFHelper();
		
		File shapeFile = null;
		List<File> textureFiles = new ArrayList<File>();
		
		for (int i = 0; i < args.length; i++) {
			if (args[i].equals("-addImage")) {
				File file = new File(args[i + 1]);
				if (file.exists() && !file.isDirectory()) {
					textureFiles.add(file);
					System.out.println("Texture File detected: " + file);
					i++;
				}
			} else if (args[i].equals("-setTemp")) {
				File file = new File(args[i + 1]);
				if (file.exists() && file.isDirectory()) {
					ofHelper.tempFolder = file;
					System.out.println("TENP_FOLDER set to " + file);
					i++;
				}
			} else if (args[i].equals("-setShape")) {
				File file = new File(args[i + 1]);
				if (file.exists() && !file.isDirectory()) {
					shapeFile = file;
		...
		if (shapeFile != null && textureFiles.size() != 0 && ofHelper.tempFolder != null) {
			
			ofHelper.makeTexturesForBlocks(shapeFile, textureFiles.toArray(new File[] {}));
	 ...
	 */
	public void makeTexturesForBlocks(File shapeFile, File[] textureFiles) {
		
		File resultFolder = new File(this.tempFolder.getParent(), "/Output/");
		if (!resultFolder.exists() || !resultFolder.isDirectory()) resultFolder.mkdir();
		
		for (File textureFile : textureFiles) {
			
			File outputFolder = makeTextures(shapeFile, textureFile);
			
			System.out.println("Copy files in output folder ...");
			File[] files = outputFolder.listFiles();
			String folderName = outputFolder.getName();
			File targetFolder = new File(resultFolder, "/" + folderName + "/");
			targetFolder.mkdir();
	 ...
	 */
	public File makeTextures(File shapeFile, File textureFile) {
		...
		for (BufferedImage image : shapeImages) {
			i++;
			writeTexture(image, new File(this.tempFolder, "/temp" + i + ".png"));
		}
		System.out.println("4 temp images saved");
		
		BufferedImage debug = combineTextures(shapeImages[1], shapeImages[3], true);
		writeTexture(debug, new File(this.tempFolder, "/debug.png"));
		...
		File outputFolder = new File(this.tempFolder, "/" + blockName + "/");
		outputFolder.mkdir();
	 ...
	 */
	public BufferedImage[] transferOverlays(BufferedImage texture, BufferedImage[] shapeOverlays) {
	 ...
	 */
	public BufferedImage readImmage(File file) {
	 ...
	 */
	public void writeTexture(BufferedImage image, File file) {
	 ...
	 */
	public BufferedImage[] cutShapeImage(BufferedImage combinedShapes) {
	 ...
	 */
	public BufferedImage[] generateShapeImages(BufferedImage[] shapeImages) {
	 ...
	 */
	public BufferedImage combineTextures(BufferedImage shape1, BufferedImage shape2, boolean invert2) {
